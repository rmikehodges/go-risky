...
AS $$
    SELECT * FROM risky_public.attack_chain_step WHERE attack_chain_id = fn_attack_chain_id OR action_id = fn_action_id;
$$ LANGUAGE sql;

CREATE OR REPLACE FUNCTION risky_public.delete_attack_chain_step(fn_attack_chain_id uuid, fn_action_id uuid)
RETURNS void 
AS $$
    DELETE FROM risky_public.attack_chain_step WHERE attack_chain_id = fn_attack_chain_id OR action_id = fn_action_id;
...
RETURNS void
AS $$
    INSERT INTO risky_public.attack_chain_step(attack_chain_id, action_id, asset_id, position, business_id) values(fn_attack_chain_id, fn_action_id, fn_asset_id,fn_position, fn_business_id);
$$ LANGUAGE sql VOLATILE;
