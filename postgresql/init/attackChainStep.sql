...
CREATE OR REPLACE FUNCTION risky_public.get_attack_chain_step(fn_attack_chain_id uuid, fn_action_id uuid, fn_asset_id uuid) 
RETURNS risky_public.attack_chain_step
AS $$
    SELECT * FROM risky_public.attack_chain_step WHERE attack_chain_id = fn_attack_chain_id AND action_id = fn_action_id AND asset_id = fn_asset_id;
$$ LANGUAGE sql;

CREATE OR REPLACE FUNCTION risky_public.delete_attack_chain_step(fn_attack_chain_id uuid, fn_action_id uuid, fn_asset_id uuid)
RETURNS void 
AS $$
    DELETE FROM risky_public.attack_chain_step WHERE attack_chain_id = fn_attack_chain_id AND action_id = fn_action_id AND asset_id = fn_asset_id;
