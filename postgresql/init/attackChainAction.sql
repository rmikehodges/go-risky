...
CREATE OR REPLACE FUNCTION risky_public.create_attack_chain_action(fn_attack_chain_id uuid, fn_action_id uuid, fn_position integer) 
RETURNS void
AS $$
    INSERT INTO risky_public.attack_chain_action(attack_chain_id, action_id, position) values(fn_attack_chain_id, fn_action_id, fn_position);
$$ LANGUAGE sql VOLATILE;


CREATE OR REPLACE FUNCTION risky_public.update_attack_chain_action(fn_attack_chain_id uuid, fn_action_id uuid, fn_position integer) 
RETURNS void 
AS $$
    UPDATE risky_public.attack_chain_action SET attack_chain_id = fn_attack_chain_id, action_id = fn_action_id, position = fn_position WHERE attack_chain_id = fn_attack_chain_id AND action_id = fn_action_id;
$$ LANGUAGE sql VOLATILE;
